...
class ApiController extends Controller
{
    ...
    public function getApiUser()
    {
        ...
        $data_from_url = $this->getApi($url);
        foreach ($data_from_url as $data_from_url) {
            if ($data_from_url->museum_code == Auth::user()->museum_code) {
                $data = array(
    ...
    public function getApiItem()
    {
        ...
        foreach ($data_url as $data_url) {
            if ($data_url->ownermuseum_code == Auth::user()->museum_code) {
                $data[] = array(
                    'sequence' => $sequence,
     ...
     */
    public function getApiGame()
    {
        // $game_id = 1;
        $sql = ApiModel::get_games();
        foreach ($sql as $sql) {
            $data[] = array(
                'game_id' => $sql->id_game,
                'game_name' => $sql->name
     ...
     */
    public function getApiCountDataset()
    {
        $matching_count = 0;
        $puzzle_count = 0;
        $quiz_count = 0;
        $mole_count = 0;
        $cosmos_count = 0;
        $item_count = $this->getCount($this->getApiItem());
        $sql = ApiModel::get_datamuseum();
        foreach ($sql as $sql) {
            if ($sql->Name == Auth::user()->museum_code) {
                if ($sql->code_game == 2) {
                    $matching_count++;
                } else if ($sql->code_game == 3) {
                    $puzzle_count++;
                } else if ($sql->code_game == 4) {
                    $quiz_count++;
                } else if ($sql->code_game == 5) {
                    $mole_count++;
                } else if ($sql->code_game == 6) {
                    $cosmos_count++;
                }
            }
        }
     ...
     * output: status
     */
    public function postApiCreateDataset(Request $request)
    {
        $sequence = 1;
        $date = date("Ymd");
        $time = time();
     ...
     */
    public function postApiSearchDataset(Request $request)
    {
        $sql = ApiModel::get_datamuseum();
     ...
     * output: status
     */
    public function postApiDeleteDataset(Request $request)
    {
     ...
     */
    public function postApiCreatGame(Request $request)
    {
